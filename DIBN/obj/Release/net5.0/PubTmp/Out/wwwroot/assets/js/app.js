...
(function ($) {
    ...
    function initLeftMenuCollapse() {
        $('.vertical-menu-btn').on('click', function (event) {
            ...
            var n = $('#PortalBalanceTable').attr("hidden");
            $('body').toggleClass('sidebar-enable');
            if ($(window).width() >= 992) {
                $('body').toggleClass('vertical-collpsed');
                if (n == null) {
                    $('#PortalBalanceTable').attr("hidden", "hidden");
                    $('#UpdateTopUpBalance').attr("hidden", "hidden");
                    $('#ViewTransactionId').attr("hidden", "hidden");
                }
                else {
                    $('#PortalBalanceTable').removeAttr("hidden");
                    $('#UpdateTopUpBalance').removeAttr("hidden");
                    $('#ViewTransactionId').removeAttr("hidden");
                }
            } else {
        ...
        $(document).ready(function () {
            if ($("#sidebar-menu").length > 0) {
                ...
                if (typeof (activeMenu) !== "undefined" && activeMenu !== null) {
                    if (activeMenu.top > 300) {
                        var activeMenu1 = activeMenu.top - 300;
                        $(".vertical-menu .simplebar-content-wrapper").animate({ scrollTop: activeMenu1 }, "slow");
                    }
                }
